...
DOCS = {
    "D": {"full_text": "Les essais de l'éclairage de secours sont effectués tous les trois mois."}
}
...
def test_quote_absent_marks_unverified():
    out = verify_quotes(
        [_conflict("la ventilation assure trente metres cubes par heure minimum")], DOCS
    )
...
def test_unknown_doc_id_does_not_fail():
    c = Conflict(
        ...
        recommendation="r",
        sources=[
            {"doc_id": "MISSING", "article": "a", "quote": "some long quote here", "value": None}
        ],
    )
