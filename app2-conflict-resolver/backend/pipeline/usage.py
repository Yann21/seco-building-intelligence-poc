...
def log_usage(pair_id: str, usage: anthropic.types.Usage) -> float:
    ...
    cost = usage.input_tokens * COST_INPUT_PER_TOKEN + usage.output_tokens * COST_OUTPUT_PER_TOKEN
    entry = {
...
def read_usage_log() -> dict:
    ...
    entries = [json.loads(line) for line in USAGE_LOG.read_text().splitlines() if line.strip()]
    return {
        "entries": entries,
